...
9. **v0.4.4 截图热键整链路失效** (2026-08-18 01:53 重启后): 实测 — Alt+X 按了无反应; 换 Control+J 能注册 (日志 `Registered screenshot shortcut: Control+J`) 但按下也无反应。**已确认的三个独立问题**: ① 启动注册吞错 (main.rs:729/740 `let _ = register()`): Windows RegisterHotKey 独占, 旧实例/其他进程占键时新实例注册静默失败, 无任何日志 → 热键死且不可诊断 (**修: 注册失败必须 log**); ② 僵尸实例嫌疑 (工作假设): 01:53 前 copy_logs 已挂死的旧实例可能仍在后台占着 Alt+X — 待任务管理器验证 img2cli.exe 进程数; ③ **热键录入器 IME 吃键**: `recordShotKeydown` 用 `e.key`, 中文 IME 下按字母 = "Process" → 录成 `Alt+Process` → 被 hotkey_rejection 拒绝 (overlay 早已换 e.code, 录入器漏改; **修: 换 e.code**)。Control+J 注册成功仍不触发 → 待按下后的日志判: 有 "Failed to capture screen" = 抓图层错; 无任何日志 = 回调/主线程层 (与 #8 同病症嫌疑)。
