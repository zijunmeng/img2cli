...
| v0.4.9 | **修复 + 手感批** (2026-08-18 重排: 排期解压后本版回归"能用好用的产品", 不再是大杂烩) | **ACL 根因修复** (`9e5e41e` 已在 dev: capabilities +`"capture"` — 浮层热窗刷新链路自 v0.4.4 以来首次真正生效, U7 可感知延迟预期随之大解) · **6-U.4 手感批**: U1 Tab 切换候选后仍可自由拖拽选区 · U2 橡皮擦圆圈光标 (半径可见, 接入工具栏 −/+) · U3 画笔/马克笔 Shift=直线 · U4 线宽档位扩大 (按工具独立记忆) · U5 贴图窗边缘 resize (6px 热区 → JS startResizeDragging, capabilities +pin-* glob) + 右键菜单修复 (右键双击触发 dblclick 关闭) · U6 保存免确认直达 (💾=一键落盘, Shift+💾=另存为) |
...
1. **Tab 之后无法再自由选区**: 实测 Tab 切换候选后,选区即被"定死",不能再任意拖拽重画; Snipaste 的 Tab 只是切换高亮候选,之后仍可随时拖拽自由选区/继续 Tab。**已修 (v0.4.9 dev, 2026-08-18)**: 破案 — 真凶是 **click-摧毁-选区**: Tab 收养后 hover 高亮被抑制 (`!hasRect` 条件), 6-P 吸附依赖的 downHover=hoverRect 变 null, 于是任何 <8px 的点击 (没拖成新选区) 把选区塌成 0×0、工具栏消失。修复: `startDraw` (两路按下的公共漏斗 — 根路径 capMouseDown + 选区内路径 rectMouseDown 的 `.stop`) 统一捕获 downHover (取自 cursorCands, 不再依赖 hover 高亮) + prevSnap {rect, confirmed}; mouseup 时 <8px 且无候选 → 恢复原选区, 有候选 → 吸附 (与无选区时行为一致)。拖拽重画/继续 Tab 本来就通, 被此 bug 掩盖。
2. **橡皮擦没有圆圈光标, 体感仍是"点击删对象"**: 期望 Snipaste 式 — 选中橡皮擦后出现 **~14px 的圆圈光标** (圈 = 实际擦除范围),按住扫过之处逐点擦除。现状: 无任何半径视觉反馈 (v0.4.4 T3 笔迹分裂已上线但 ERASE_RADIUS=10 看不见; 形状/文本/马赛克整删属设计)。**已修 (v0.4.9 dev)**: 圆圈光标 (白圈+黑描边, 跟随鼠标, 半径=橡皮擦 size) + 原生光标隐藏 (cursor-none, 选区边框同步) + 半径接入 −/+ (4–60)。
3. **画笔/马克笔 Shift = 直线**: 按住 Shift 拖拽应绘制从落点到当前点的直线 (通用绘图惯例)。**已修 (v0.4.9 dev)**: 45° 吸附 shiftSnap — 笔/马克笔 = 锚点直线 (替换式两点笔迹), 箭头 = 角度吸附, 矩形/椭圆/马赛克 = 正方形; 松开 Shift 续画。提示行加了 "Shift = 直线/正方形"。
4. **粗细/大小调节不足**: 画笔/马克笔线宽当前只有共享 toolSize 1–8,范围太窄; 橡皮擦半径固定 10px,未接进工具栏 −/+。需: 线宽档位扩大 (或按工具独立记忆), 橡皮擦大小可调。**已修 (v0.4.9 dev)**: 按工具独立记忆 (reactive map) + 各自量程: 笔 1–20 / 马克笔 1–12 (×4 像宽) / 箭头·矩形·椭圆 1–12 / 马赛克 2–20 (块 = max(4,3s)) / 文本 2–20 (字号 = 12+3s) / 橡皮擦 4–60; −/+ 只调当前工具, select 态禁用变灰。
5. **贴图窗边缘 resize 无效 + 右键直接关闭**: 实测仍只能移动 + 滚轮缩放 — 窗口虽 `resizable(true)` (main.rs T7 注释),undecorated 窗口边缘命中不生效,需 WM_NCHITTEST 边缘热点或显式缩放手柄; 右键未弹菜单而是把贴图关掉了。**已修 (v0.4.9 dev)**: ①右键关闭破案 — **Chromium 对任意按键双击都派发 dblclick**, 右键连点触发 `@dblclick="closePin"` → 只认左键 (button===0); 拖拽/菜单同步只认左键。②边缘 resize — 6px 热区 (JS mousemove 检测 + 方向光标) → mousedown 调 JS 官方 `getCurrentWindow().startResizeDragging('North'/…)` 交给 OS 原生缩放循环; **ACL 配套**: capabilities 加 `pin-*` (glob) + `core:window:allow-start-resize-dragging` (core:window:default 是只读集, 不含它)。曾试 Rust 侧自命令但 `WebviewWindow` 不暴露 start_resize_dragging (window 句柄 pub(crate), Manager::get_window 要 unstable feature) — JS 插件路更干净。③滚轮缩放改以实际窗口尺寸为基准 (edge-resize 拉伸后缩放不漂移)。
6. **保存应免确认直达另存为**: 期望选区存在即可点 💾 直接弹"另存为"对话框,不需要先点 ✓; 实测必须先点对勾 (机制待复现)。注意 v0.4.4 T5 把 💾 改成了**无对话框一键落盘** — 与本次期望方向冲突,需决策。**已修 (v0.4.9 dev, 方案=💾一键 + Shift+💾=另存为)**: actionSave 免确认 (选区存在即用, 含未确认态 — 原本就不校验 confirmed, "必须先点对勾"实为用户在找对话框反馈; 无选区时按钮本就不存在); 普通点击 = T5 一键自动命名落盘+退出, Shift+点击 = Rust save_image_dialog 另存为; tooltip 标注两种用法。
...
**★ 破案 (2026-08-18 21:07 v0.4.8 现场 log + 代码考古, 根因 = ACL, 修复一行, 已推 dev 待装机验证)**: 实测 — 启动后 `get_captured_image called (overlay webview alive)` 出现于 21:06:59 (prewarm webview 的 mount JS 跑通, 自定义命令不受 ACL 管); 按 Alt+X → 前三行出、`get_captured_image`/`Capture overlay shown` 永不再出; 第二按触发判死重建 → `window build failed: a webview with label 'capture' already exists` (close() 异步拆 label, 下一次按键才完成重建 — capture.rs 注释里明示的两步设计, 独立小问题)。**代码考古**: v0.4.2 的浮层是 Rust 侧直接 `win.show()` (App.vue 无任何 capture-refresh listen, 实测可用); `listen('capture-refresh')` 系 v0.4.4 T6 (常驻热窗) 引入; `capabilities/default.json` 自项目初始就是 `windows:["main"]` (git log --follow 仅 3 commit, 从未加过 capture); **v0.4.2 已证同机制**: overlay 窗口调 dialog 插件被 ACL 静默拒 (当时的修复 = 挪 Rust 自定义命令); main 窗口的 `log_append` listen 一直正常 (core:default 授 event)。**根因**: capture 窗口不在任何 capability 里 → mount 时 `listen('capture-refresh')` 被 ACL 拒 → 热窗刷新链路**从未在任何版本生效过**; "webview 死亡/挂起"系误诊 — **webview 活着, 但是聋的**。**历史重新解读**: v0.4.4 当晚能用浮层 (工作假设, 无日志可证): v0.4.4 的 prewarm 失败是静默的 (`let _ =`, v0.4.5 才加日志) → 每次按键走 fresh-build 兜底 (mount 直拉图片, 不需要事件) → 能出但慢 — U7 "可感知延迟"的真身就是这条兜底路径; v0.4.6 修好 prewarm 后热窗真正建立 → 从此 100% 走聋路径 → 永久死。v0.4.7 看门狗 / v0.4.8 按键自愈都是在给一个不存在的"死 webview"做心肺复苏。**修复**: capabilities `windows` 加 `"capture"`; 装机后 Alt+X 秒出浮层 + daemon.log 四行全出 = 根因坐实 (终局实验)。**终局实验结果 (2026-08-18 22:52, 打点版 dev 构建)**: ACL 修复**证实生效** — 按键后 `get_captured_image called` 首次出现在按键链路上 (此前一天从未出现), 事件送达/监听器/JS/invoke 全通; 但断点后移: `loadCaptureImage FAILED: JPEG encode failed: ... does not support the color type Rgba8`。**第三层根因 (已证)**: JPEG 无 alpha 通道, 显示层编码器从 v0.4.4 起直接喂 Rgba8 — 该分支被 ACL 聋死掩盖, **是真机上从未执行过一次的死代码**, ACL 修好后首次运行即当场失败 (CI 只编译不运行, 本机无 Windows 图形环境, 属"从未被运行时验证"教训)。修复 `36825ed`: 编码前剥 alpha 进紧凑 RGB 缓冲 (截图全不透明, 显示无损; clipboard.rs 两处编码一直是正确写法 to_rgb8, 该模式没进 capture.rs)。**6-U.11 悬案随之关闭**: webview 从未系统性死亡, WebView2/驱动/UDF 嫌疑撤销; README 排障指引保留 (无害)。
...
10. **v0.4.5 启动 "state not managed" 回归的三个连锁现象** (2026-08-18 用户报告, 回归本体**已修 v0.4.6** `a404b99`): ① 热键"变回默认 Alt+Shift+S" — get_config 启动失败 → 前端 `config` 保持 JS 默认对象, 界面显示默认值 (**非磁盘配置被改**; 磁盘仍是 Rust 侧独立加载的 Alt+X, 启动注册不受影响); ② **改不回 Alt+X** — 录入器只收到 Alt (裸修饰键): IME 在 `<input>` 里把字母 keydown 整个吞掉 (v0.4.5 的 Process 回退只救"到达的 Process 事件", 字母事件根本没到达 recorder) → 存成裸 `Alt` → 被 hotkey_rejection 拒 ("without modifiers must be a function key", 报错本身合理)。**已修 (v0.4.7)**: keyup 兜底 — 修饰键挂起时幸存的 keyUP 补齐组合; ③ **隐性配置覆盖风险**: get_config 失败状态下若用户点过一次成功的"保存设置", 默认值 config 会整份写回 config.toml (targets/热键/主题全部归零) — v0.4.6 装上后需检查 Hosts & Targets 是否还在。**已修 (v0.4.7)**: `configLoaded` 闸 — 加载失败禁用 Save。
11. **v0.4.7 看门狗把"浮层不出"升级成"整机卡死"** (2026-08-18 用户报告 → **已修 v0.4.8** `824c367`): 实测 — Alt+X 不出浮层, 且软件整体卡死 (打不开日志、托盘右键无响应 = 主线程堵死)。**根因 (已证)**: v0.4.7 的后台看门狗在"webview 死"的机器上反复 close+build —— 每次 build 都在主线程创建 WebView2, 死 webview 的销毁/创建堆积 → 主线程永久阻塞。**教训入库**: ① 永远不要从后台线程自动做窗口手术 — 窗口操作只在用户节奏 (按键) 里做, 每次至多一次; ② 日志只存内存 = 冻死后取证不能 → daemon 日志必须镜像落盘。**v0.4.8 方案**: emits/shows 计数判死 (emit 过的刷新从未到达 shown = 热窗死) + 下一次按键重建 + `%TEMP%/img2cli/daemon.log` 镜像。**悬案 (v0.4.9)**: 该机器上 webview 为何系统性死亡 (WebView2 挂起 / GPU 驱动 / UDF 损坏均未排除; 排障指引已写进 README Troubleshooting)。
